...
@Field private static final String MLD_DEPLOYED_VERSION = "0.3.1"
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", uninstall: true, install: true) {
        section("About Modern Dashboard", hideable: true, hidden: true) {
            ...
            paragraph "<small>Version 0.3.1 · Ephrayim (evdev) · Apache License 2.0 · <a href='https://github.com/evdev/hubitat-modern-dashboard' target='_blank'>Source</a></small>"
        }
        if (assetsOk) {
            ...
            section("Required: upload dashboard files") {
                ...
                if (!hubSecurity) {
                    ...
                    paragraph "<small><b>Hub file access:</b> login failed — verify hub admin username/password under <b>Hub file access</b> below.</small>"
                }
            }
        }
...
def hubAuthCookie(boolean forceRefresh = false) {
    ...
    try {
        ...
        if (loginFailed || !cookie) {
            state.remove("hubAuthCookie")
            state.remove("hubAuthCookieExpiresAt")
            log.error "hubAuthCookie: hub login failed — verify Hub file access username/password"
            return null
        }
